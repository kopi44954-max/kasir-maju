...
export default function NexusPOS() {
  ...
  const updateQty = (id: number, delta: number, stock: number) => {
    setCart(prev => {
      const updated = prev.map(i => {
        if (i.id === id) {
          const newQty = Math.min(Math.max(0, i.qty + delta), stock);
          return { ...i, qty: newQty };
        }
        return i;
      });
      // Jika qty 0 setelah ditekan tombol minus, hapus dari keranjang
      return updated.filter(i => i.qty > 0);
    });
  };

  // LOGIKA INPUT MANUAL YANG DIPERBAIKI
  ...
  const handleBlur = (id: number, qty: any) => {
    // Jika saat kursor keluar input kosong atau 0, hapus dari keranjang
    if (qty === "" || qty === 0) {
            ...
            {cart.map(i => (
              <div key={i.id} className="bg-white/[0.02] p-4 rounded-xl border border-white/5">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <p className="text-sm font-bold text-white italic">Rp{( (Number(i.qty) || 0) * i.price).toLocaleString()}</p>
                </div>
              </div>
            ))}
        </div>
