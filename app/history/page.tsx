...
export default function LaporanKeuangan() {
  ...
  const stats = filteredData.reduce((acc, curr) => {
    ...
    const modal = curr.items.reduce((sum: number, item: any) => sum + (Number(item.costPrice || 0) * item.qty), 0);
    acc.omzet += omzet;
    acc.modal += modal;
    acc.laba += (omzet - modal);
    return acc;
  ...
  return (
    <div className="min-h-screen bg-[#07080A] text-slate-400 font-sans pb-20 selection:bg-emerald-500/30">
      <div className="max-w-6xl mx-auto p-6 md:p-12">
        ...
        {/* SUMMARY CARDS - ELEGAN & MINIMALIS */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
          {[
            { label: "Pendapatan", value: stats.omzet, color: "text-white" },
            { label: "Pengeluaran Modal", value: stats.modal, color: "text-rose-500" },
            { label: "Laba Bersih", value: stats.laba, color: "text-emerald-500" }
          ].map((item, i) => (
            <div key={i} className="bg-[#111318] border-l-2 border-white/5 p-8 hover:border-emerald-500 transition-all duration-500">
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-600 mb-4">{item.label}</p>
          ...
          <div className="bg-[#111318] border border-white/5 rounded-b-xl overflow-hidden">
            {loading ? (
              ...
              <div className="py-24 text-center space-y-3">
                ...
                <p className="text-xs font-bold uppercase tracking-widest text-slate-700">Data Kosong</p>
              </div>
            ) : (
              <div className="divide-y divide-white/5">
                {filteredData.reverse().map((t, idx) => {
                  const tModal = t.items.reduce((s: number, i: any) => s + (Number(i.costPrice || 0) * i.qty), 0);
                  const tLaba = t.totalPrice - tModal;
