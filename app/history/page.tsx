...
export default function HistoryGlass() {
  ...
  return (
    <div className="h-screen bg-slate-50 flex flex-col md:flex-row font-sans overflow-hidden">
      ...
      {/* SIDEBAR (Desktop) - Fixed gaya app/page.tsx */}
      <aside className="hidden md:flex w-20 bg-white border-r border-slate-200 flex-col items-center py-8 gap-6 h-full">
        ...
        <button onClick={() => window.location.href='/setting'} className="p-3 text-slate-400 hover:text-blue-600 transition-all"><Settings size={22}/></button>
      </aside>

      {/* BOTTOM NAV (Mobile) - Fixed gaya app/page.tsx */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 flex justify-around items-center h-16 z-50">
        <button onClick={() => window.location.href='/'} className="flex flex-col items-center text-slate-400">
            <Home size={20}/><span className="text-[10px] font-bold mt-1">Home</span>
        </button>
        <button onClick={() => window.location.href='/history'} className="flex flex-col items-center text-orange-600">
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
              ...
              <p className="text-xl font-black text-slate-800">Rp{calculateTotal('total').toLocaleString()}</p>
            </div>
            <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm border-l-4 border-l-green-500">
              <p className="text-[9px] font-black text-green-600 uppercase mb-1">Laba Bersih</p>
              <p className="text-xl font-black text-green-600">Rp{calculateTotal('profit').toLocaleString()}</p>
            </div>
          </div>

          <div className="space-y-3">
            <h3 className="font-black text-xs uppercase tracking-widest text-slate-500 flex items-center gap-2 px-1">
              <Clock size={16}/> Log Penjualan Terbaru
            </h3>
            ...
            <AnimatePresence>
              {[...transactions].reverse().map((t) => (
                <motion.div 
                  ...
                  className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col gap-3 hover:border-orange-200 transition-all"
                >
                  <div className="flex justify-between items-start">
                    ...
                    <button onClick={() => deleteTx(t.id)} className="p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 transition-all rounded-lg">
                      <Trash2 size={16}/>
                    </button>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {t.items?.map((it:any, i:number) => (
                      <span key={i} className="px-2 py-1 bg-slate-50 border border-slate-100 text-[10px] font-bold rounded-lg uppercase text-slate-600">
                        {it.name} <span className="text-orange-500 ml-1">x{it.qty}</span>
                      </span>
                    ))}
                  </div>

                  <div className="pt-3 border-t border-slate-50 flex justify-between items-center">
                     <span className="text-[9px] font-black text-slate-400 uppercase">Profit Transaksi:</span>
